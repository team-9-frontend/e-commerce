...
return ( <div className="w-full flex-1 space-y-3 md:space-y-6">
{isError ? ( <div className="flex flex-col items-center justify-center py-30 text-center"> <h3 className="text-lg font-bold text-red-600">Failed to load products</h3> <p className="mt-2 text-sm text-neutral-500">
{error?.message || 'Something went wrong. Please try again.'} </p> </div>
) : products.length === 0 && !isLoading ? ( <div className="flex flex-col items-center justify-center py-30 text-center"> <h3 className="text-lg font-bold text-neutral-600">No Products Found</h3> </div>
) : (
...
: products.map((product) => ( <StoreProductCard
                 key={product._id}
                 product={product}
...
>
{/* Image */} <div className="relative aspect-square w-full overflow-hidden bg-neutral-50 dark:bg-neutral-800"> <img
...
return ( <div className="flex w-full flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-800"> <div className="aspect-square w-full"> <Skeleton height="100%" className="block h-full w-full" /> </div>
